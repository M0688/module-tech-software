...
const DEFAULT_FLOW_TEMPLATE = {
  title: "ECU board inspection & fault diagnosis",
  category: "ECU repair",
  summary: "Complete bench workflow — intake, visual inspection, power rails, boot signals, CAN/LIN, outputs, thermal checks, and post-repair verification.",
  steps: [
    {
      title: "Phase 1 — Intake & bench setup",
      detail: "Collect symptom information (no start, intermittent shutdown, cooling fan always on, fuel pump not operating, no diagnostic communication, stored DTCs). Connect ECU to regulated 12–13.8 V DC supply with current limit 0.7–1.0 A (increase to 2–3 A only when testing outputs). Use anti-static wrist strap, ESD mat, and gloves.",
      fields: [
        { label: "Symptoms / DTCs reported", type: "textarea" },
        { label: "ESD precautions in place", type: "passfail" },
        { label: "Bench supply voltage", type: "number", min: 12, max: 13.8, unit: "V" },
        { label: "Current limit set (0.7–1.0 A idle)", type: "passfail" },
      ],
    },
    {
      title: "Phase 2 — Visual board inspection (before power)",
      detail: "Look for burn marks, soot, corrosion/water ingress, cold solder joints (especially main connector and heavy components), cracked PCB or vias. Brown/black spots or burnt odor often indicate failed regulators, TVS diodes, MLCCs, or output drivers.",
      fields: [
        { label: "No burn marks or soot", type: "passfail" },
        { label: "No corrosion or water ingress", type: "passfail" },
        { label: "No cold solder at connector / heavy parts", type: "passfail" },
        { label: "No cracked PCB or vias", type: "passfail" },
        { label: "TVS diode — no crack/blackening near Vbat", type: "passfail" },
        { label: "Capacitors — no bulging/leakage (tantalum/electrolytic)", type: "passfail" },
        { label: "MLCCs — no hairline cracks or dark spots", type: "passfail" },
        { label: "CAN choke / MOSFET-IGBT — no burn damage", type: "passfail" },
        { label: "Board photo", type: "photo" },
        { label: "Visual fault notes", type: "textarea" },
      ],
    },
    {
      title: "Phase 2 — Board areas (connector inward)",
      detail: "Work from the ECU connector inward: input protection (TVS, reverse-polarity diode, fuse), voltage regulators (5 V / 3.3 V / 1.8 V rails), ground connections, processing core (supervisor, crystal, MCU), memory (Flash/EEPROM), communications (CAN/LIN transceivers), analog sensor inputs, actuator outputs (injector, ignition, relay, fan drivers).",
      fields: [
        { label: "Input protection intact (TVS, fuse)", type: "passfail" },
        { label: "Regulator section — no obvious damage", type: "passfail" },
        { label: "Ground paths & connector pins OK", type: "passfail" },
        { label: "Crystal / MCU area OK", type: "passfail" },
        { label: "Memory devices (Flash/EEPROM) OK", type: "passfail" },
        { label: "CAN/LIN transceiver area OK", type: "passfail" },
        { label: "Output driver section OK", type: "passfail" },
      ],
    },
    {
      title: "Phase 3 — Power & current checks",
      detail: "Power the ECU with current limit enabled. Normal idle draw: 80–300 mA. If excessive, use thermal camera or freeze spray to locate hot spots; measure resistance to ground on each rail — the lowest-resistance rail is usually the fault path. Burnt TVS reads near-zero Ω between Vbat and GND.",
      fields: [
        { label: "Idle current draw", type: "number", min: 80, max: 300, unit: "mA" },
        { label: "Resistance Vbat–GND (no short)", type: "number", min: 1000, unit: "Ω" },
        { label: "Ground integrity (ECU GND–chassis)", type: "number", max: 0.1, unit: "Ω" },
        { label: "+5 V rail", type: "number", min: 4.9, max: 5.1, unit: "V" },
        { label: "+3.3 V rail", type: "number", min: 3.25, max: 3.35, unit: "V" },
        { label: "5 V ripple", type: "number", max: 100, unit: "mVpp" },
        { label: "All expected supply rails present", type: "passfail" },
      ],
    },
    {
      title: "Phase 4 — Boot process & vital signals",
      detail: "RESET should transition Low → High after power-up (stuck Low = voltage supervisor, watchdog, or MCU fault). Clock: steady oscillation at 4–40 MHz on crystal pins — no clock means MCU will not boot. During the first 50–300 ms after power-on, observe CS/SCK/MOSI/MISO for memory bus activity.",
      fields: [
        { label: "RESET rises after power-up", type: "passfail" },
        { label: "Crystal oscillating (4–40 MHz)", type: "passfail" },
        { label: "Flash/EEPROM bus activity on boot", type: "passfail" },
        { label: "MCU overheating without load (visual/suspect)", type: "passfail" },
        { label: "Crystal can — no mechanical damage", type: "passfail" },
      ],
    },
    {
      title: "Phase 5 — Network & communication",
      detail: "CAN bus: recessive ≈ 2.5 V / 2.5 V, dominant ≈ 3.5 V / 1.5 V. Measure 60 Ω between CAN_H and CAN_L (two 120 Ω terminations in parallel). Observe TXD/RXD between MCU and transceiver. LIN: idle ≈ battery voltage (~12 V), dominant = 0 V.",
      fields: [
        { label: "CAN_H–CAN_L resistance", type: "number", min: 55, max: 65, unit: "Ω" },
        { label: "CAN recessive voltages OK (~2.5 V both)", type: "passfail" },
        { label: "CAN dominant voltages OK (~3.5 V / 1.5 V)", type: "passfail" },
        { label: "TXD/RXD activity (MCU ↔ transceiver)", type: "passfail" },
        { label: "LIN bus signals OK", type: "passfail" },
        { label: "Common-mode choke / transceiver — no burn damage", type: "passfail" },
      ],
    },
    {
      title: "Phase 6 — Analog sensor inputs",
      detail: "Trace: connector → RC filter / clamp diodes → op-amp → ADC. Inject known reference voltages (1.00 V, 2.50 V, 4.00 V) and compare with expected readings. Noisy/step readings → filter caps, analog ground, op-amp. Maxed reading (>4.9 V) → clamp diode leak or reference error.",
      fields: [
        { label: "Reference 1.00 V reads correctly", type: "passfail" },
        { label: "Reference 2.50 V reads correctly", type: "passfail" },
        { label: "Reference 4.00 V reads correctly", type: "passfail" },
        { label: "No stuck/max ADC readings (>4.9 V)", type: "passfail" },
        { label: "Analog input notes (MAP, IAT, ECT, O2, etc.)", type: "textarea" },
      ],
    },
    {
      title: "Phase 7 — Outputs & actuators",
      detail: "Injector driver: test with dummy load (12 V / 21 W lamp) — look for clean square pulse. Ignition coil driver: observe dwell waveform and cut-off spike. Relay/fan/pump: low-side should pull to 0 V, high-side should rise to Vbat. One channel failed → suspect that driver or trace.",
      fields: [
        { label: "Injector driver output OK", type: "passfail" },
        { label: "Ignition coil driver OK", type: "passfail" },
        { label: "Relay / fan / pump drivers OK", type: "passfail" },
        { label: "No output stuck to Vbat or GND", type: "passfail" },
        { label: "Output test notes", type: "textarea" },
      ],
    },
    {
      title: "Phase 8 — Thermal, mechanical & moisture",
      detail: "Profile temps with thermal camera — >70–90 °C at idle is suspicious. Cold solder/cracks: around connectors, heavy components, PCB corners (reflow with flux). Moisture/corrosion: green deposits, bubbles under conformal coating — clean with IPA and dry thoroughly.",
      fields: [
        { label: "Max idle temperature", type: "number", max: 90, unit: "°C" },
        { label: "No suspicious hot spots at idle", type: "passfail" },
        { label: "No cold solder / mechanical cracks", type: "passfail" },
        { label: "No moisture / corrosion under coating", type: "passfail" },
      ],
    },
    {
      title: "Phase 9 — Memory & firmware",
      detail: "EEPROM: immobilizer/adaptation faults cause no-start or erratic behaviour — read in-circuit if possible and compare with known-good dump. Flash: if bus is active but ECU fails to boot, reflash with correct firmware.",
      fields: [
        { label: "EEPROM read / compare OK", type: "passfail" },
        { label: "Flash content / boot sequence OK", type: "passfail" },
        { label: "Memory / firmware notes", type: "textarea" },
      ],
    },
    {
      title: "Phase 10 — Symptom → likely cause",
      detail: "Map reported symptoms to likely causes: no comms → missing 5 V, CAN/LIN transceiver, termination, MCU boot. No start → immobilizer EEPROM, CKP sensor, ignition/fuel drivers. High idle current (>300 mA) → MLCC short, TVS failure, burnt driver. Fan always on → high-side driver shorted.",
      fields: [
        { label: "Primary symptom", type: "text" },
        { label: "Likely fault area identified", type: "text" },
        { label: "Components flagged for replacement", type: "textarea" },
      ],
    },
    {
      title: "Phase 11 — Repair & verification",
      detail: "Replace faulty components with automotive-grade −40…+125 °C parts; reinforce burnt traces with copper jumpers. Repeat power, boot, and comms checks post-repair. On-vehicle: clear DTCs, full drive cycle, verify live data and operation (cold/hot starts, under load).",
      fields: [
        { label: "Repairs completed", type: "passfail" },
        { label: "Post-repair rails & waveforms recorded", type: "passfail" },
        { label: "On-vehicle test completed", type: "passfail" },
        { label: "DTCs cleared & verified", type: "passfail" },
        { label: "Final verification notes", type: "textarea" },
      ],
    },
  ],
};

const DEFAULT_FLOW_STEPS = DEFAULT_FLOW_TEMPLATE.steps;

function evalMeasValue(num, min, max) {
  if (Number.isNaN(num)) return null;
  return (min == null || num >= min) && (max == null || num <= max);
}

function formatMeasRange(field) {
  const u = field.unit ? " " + field.unit : "";
  const parts = [];
  if (field.min != null) parts.push("≥ " + field.min + u);
  if (field.max != null) parts.push("≤ " + field.max + u);
  return parts.join(" and ");
}

function fieldStoredValue(v) {
  if (v == null) return "";
...
function fieldStoredPass(v, field) {
  if (field.type === "passfail") {
    if (typeof v === "object" && v != null && "pass" in v) return !!v.pass;
  ...
  if (field.type === "number" && (field.min != null || field.max != null)) {
    if (typeof v === "object" && v != null && "pass" in v) return !!v.pass;
    const raw = fieldStoredValue(v);
    if (raw === "") return null;
    return evalMeasValue(Number(raw), field.min ?? null, field.max ?? null);
  }
  return null;
}

function measFlagHtml(pass) {
  if (pass == null) return `<span class="meas-flag"></span>`;
  return `<span class="meas-flag ${pass ? "pass" : "fail"}">${pass ? "✓" : "✗"}</span>`;
}
...
window.seedEcuDiagnostic = async () => {
  const { data: existing } = await db.from("diagnostic_flows").select("id").eq("title", DEFAULT_FLOW_TEMPLATE.title).maybeSingle();
  if (existing) {
    toast("ECU template already exists — opening it", "success");
    location.hash = "diagnostics/edit/" + existing.id;
    return;
  }
  const { data, error } = await db.from("diagnostic_flows").insert({
    title: DEFAULT_FLOW_TEMPLATE.title,
    category: DEFAULT_FLOW_TEMPLATE.category,
    summary: DEFAULT_FLOW_TEMPLATE.summary,
    steps: JSON.parse(JSON.stringify(DEFAULT_FLOW_TEMPLATE.steps)),
  }).select("id").single();
  if (error) return toast(error.message, "error");
  toast("ECU diagnostic template created", "success");
  location.hash = "diagnostics/edit/" + data.id;
};
...
views.diagnostics = async (rest) => {
  ...
  el("view").innerHTML = `
    <div class="page-head"><div><h1>Diagnostics</h1>
      <div class="page-sub">ECU bench checklists with live pass/fail on measurements</div></div>
      <div class="row-actions">
        <button class="btn" onclick="seedEcuDiagnostic()">+ ECU guide template</button>
        <button class="btn btn-primary" onclick="location.hash='diagnostics/new'">+ New template</button>
      </div></div>
...
window.evalMeas = (input) => {
  const row = input.closest(".diag-field-row") || input.parentElement;
  const flag = row ? row.querySelector(".meas-flag") : input.parentElement.querySelector(".meas-flag");
  if (!flag) return;
  const v = input.value.trim();
  input.classList.remove("input-pass", "input-fail");
  if (v === "") { flag.textContent = ""; flag.className = "meas-flag"; updateDiagProgress(); return; }
  const num = Number(v);
  const min = input.dataset.min !== "" ? Number(input.dataset.min) : null;
  const max = input.dataset.max !== "" ? Number(input.dataset.max) : null;
  const pass = evalMeasValue(num, min, max);
  flag.textContent = pass ? "✓" : "✗";
  flag.className = "meas-flag " + (pass ? "pass" : "fail");
  input.classList.add(pass ? "input-pass" : "input-fail");
  updateDiagProgress();
};

window.setPassFail = (btn, pass) => {
  const wrap = btn.closest(".passfail-btns");
  if (!wrap) return;
  const hidden = wrap.querySelector("input[type=hidden]");
  const passBtn = wrap.querySelector(".pf-pass");
  const failBtn = wrap.querySelector(".pf-fail");
  const flag = wrap.querySelector(".meas-flag");
  hidden.value = pass ? "pass" : "fail";
  passBtn.classList.toggle("active", pass);
  passBtn.classList.toggle("pass", pass);
  failBtn.classList.toggle("active", !pass);
  failBtn.classList.toggle("fail", !pass);
  if (flag) {
    flag.textContent = pass ? "✓" : "✗";
    flag.className = "meas-flag " + (pass ? "pass" : "fail");
  }
...
window.toggleDiagStep = (si) => {
  const panel = el(`diag-step-${si}`);
  const chk = el(`step_done_${si}`);
  if (panel && chk) panel.classList.toggle("step-done", chk.checked);
  updateDiagProgress();
};

window.updateDiagProgress = () => {
  const form = el("run-form");
  if (!form || !window._diagRun) return;
  const { steps } = window._diagRun;
  let total = 0;
  let filled = 0;
  let passing = 0;
  let failing = 0;
  steps.forEach((s, si) => {
    (s.fields || []).forEach((field, fi) => {
      if (field.type === "photo") return;
      total++;
      const inp = el(`f_${si}_${fi}`);
      if (!inp) return;
      if (field.type === "passfail") {
        if (inp.value === "pass") { filled++; passing++; }
        else if (inp.value === "fail") { filled++; failing++; }
      } else if (field.type === "number" && (field.min != null || field.max != null)) {
        const v = inp.value.trim();
        if (v !== "") {
          filled++;
          const pass = evalMeasValue(Number(v), field.min ?? null, field.max ?? null);
  ...
  steps.forEach((s, si) => {
    const statusEl = el(`step-status-${si}`);
    const panel = el(`diag-step-${si}`);
    ...
    (s.fields || []).forEach((field, fi) => {
      ...
      if (!inp) return;
      if (field.type === "passfail") {
        if (inp.value === "pass") stepPass++;
        else if (inp.value === "fail") stepFail++;
        else stepEmpty++;
      } else if (field.type === "number" && (field.min != null || field.max != null)) {
        const v = inp.value.trim();
        if (v === "") stepEmpty++;
        else if (evalMeasValue(Number(v), field.min ?? null, field.max ?? null)) stepPass++;
        else stepFail++;
      } else if (inp.value && inp.value.trim()) stepPass++;
      else stepEmpty++;
    });
    panel.classList.remove("step-pass", "step-fail");
    if (stepFail > 0) {
      statusEl.innerHTML = measFlagHtml(false);
      panel.classList.add("step-fail");
    } else if (stepEmpty === 0 && stepPass > 0) {
      statusEl.innerHTML = measFlagHtml(true);
      panel.classList.add("step-pass");
    } else {
      statusEl.innerHTML = `<span class="meas-flag"></span>`;
    }
  });
  const pct = total ? Math.round(filled / total * 100) : 0;
  if (el("diag-progress-fill")) el("diag-progress-fill").style.width = pct + "%";
  if (el("diag-progress-label")) {
    let label = `${filled} of ${total} checks entered`;
    if (failing) label += ` · ${failing} failing`;
    el("diag-progress-label").textContent = label;
  }
  if (el("diag-progress-pct")) el("diag-progress-pct").textContent = pct + "%";
};
...
async function flowRunForm(flowId, presetJobId) {
  ...
  const fieldInput = (si, fi, field) => {
    const id = `f_${si}_${fi}`;
    if (field.type === "photo") return `<input id="${id}" type="file" accept="image/*,application/pdf" onchange="updateDiagProgress()">`;
    if (field.type === "textarea") return `<textarea id="${id}" oninput="updateDiagProgress()"></textarea>`;
    if (field.type === "passfail") {
      return `<div class="passfail-btns">
        <button type="button" class="pf-btn pf-pass" onclick="setPassFail(this,true)">Pass ✓</button>
        <button type="button" class="pf-btn pf-fail" onclick="setPassFail(this,false)">Fail ✗</button>
        <input type="hidden" id="${id}" value="">
        <span class="meas-flag"></span>
      </div>`;
    }
    if (field.type === "number") {
      const hasRange = field.min != null || field.max != null;
      const unitHtml = field.unit ? `<span class="meas-unit">${esc(field.unit)}</span>` : "";
      if (!hasRange) {
        return `<div class="meas"><input id="${id}" type="number" step="any" oninput="updateDiagProgress()">${unitHtml}</div>`;
      }
      return `<div class="meas"><input id="${id}" type="number" step="any" data-min="${field.min ?? ""}" data-max="${field.max ?? ""}" oninput="evalMeas(this)">${unitHtml}<span class="meas-flag"></span></div>
        <div class="meas-range muted">Good: ${esc(formatMeasRange(field))}</div>`;
    }
    return `<input id="${id}" type="text" oninput="updateDiagProgress()">`;
  };

  const fieldRow = (si, fi, field) => `<div class="diag-field-row" id="field_${si}_${fi}">
    ...
    <form id="run-form">
      ...
      <div class="diag-progress-wrap">
        <div class="diag-progress-head">
          <span id="diag-progress-label">0 of 0 checks entered</span>
          <span id="diag-progress-pct" class="muted">0%</span>
        </div>
        <div class="diag-progress-bar"><div class="diag-progress-fill" id="diag-progress-fill"></div></div>
      </div>
      ${steps.map((s, si) => `
        <div class="panel diag-step" id="diag-step-${si}">
          <div class="diag-step-head">
            <label class="diag-step-check">
              <input type="checkbox" id="step_done_${si}" onchange="toggleDiagStep(${si})">
              <span class="step-num">${si + 1}</span>
            </label>
            <div class="diag-step-title-wrap">
              <div class="step-title">${esc(s.title)}</div>
              ${s.detail ? `<div class="step-detail">${esc(s.detail)}</div>` : ""}
            </div>
            <div class="diag-step-status" id="step-status-${si}"><span class="meas-flag"></span></div>
          </div>
          <div class="diag-fields">
            ${(s.fields || []).map((field, fi) => fieldRow(si, fi, field)).join("")}
  ...
  $("#run-form").addEventListener("submit", async (e) => {
    ...
    for (let si = 0; si < steps.length; si++) {
      ...
      for (let fi = 0; fi < (s.fields || []).length; fi++) {
        ...
        } else if (field.type === "passfail") {
          if (inp.value === "pass" || inp.value === "fail") {
            values[`s${si}f${fi}`] = { pass: inp.value === "pass" };
          }
        } else if (field.type === "number" && (field.min != null || field.max != null)) {
          const v = inp.value.trim();
          if (v !== "") {
            const pass = evalMeasValue(Number(v), field.min ?? null, field.max ?? null);
            values[`s${si}f${fi}`] = { value: v, pass: !!pass };
          }
      ...
      const stepDone = el(`step_done_${si}`);
      if (note && note.value.trim()) values[`s${si}note`] = note.value.trim();
      if (stepDone && stepDone.checked) values[`s${si}done`] = true;
    }
...
async function flowRunView(runId) {
  ...
  steps.forEach((s, si) => {
    (s.fields || []).forEach((field, fi) => {
      ...
      if (v == null) return;
      const pass = fieldStoredPass(v, field);
      if (pass != null) {
        totalChecks++;
  ...
  el("view").innerHTML = `
    ...
    <div class="page-head"><div><h1>${esc(run.title || "Diagnostic")}</h1>
      <div class="page-sub">Recorded ${fmtDate(run.created_at)}${totalChecks ? ` · ${passCount} passed${failCount ? `, ${failCount} failed` : ""}` : ""}</div></div>
      <div class="row-actions"><button class="btn btn-danger" onclick="deleteRow('diagnostic_runs','${run.id}','${backJob}')">Delete</button></div></div>
    ${steps.map((s, si) => {
      const stepDone = values[`s${si}done`];
      let stepPass = 0;
      let stepFail = 0;
      const rows = (s.fields || []).map((field, fi) => {
        const v = values[`s${si}f${fi}`];
        if (v == null) return "";
        if (field.type === "photo" && v.photo) return `<div class="diag-field-row"><div class="field full"><label>${esc(field.label)}</label>
          <img class="diag-photo" data-path="${esc(v.photo)}" alt="${esc(field.label)}" onclick="openFile('${esc(v.photo)}')" title="Click to open"></div></div>`;
        if (field.type === "passfail") {
          const pass = fieldStoredPass(v, field);
          if (pass != null) { if (pass) stepPass++; else stepFail++; }
          return `<div class="diag-field-row"><div class="field"><label>${esc(field.label)}</label>
            <div class="ro-val">${pass ? "Pass" : "Fail"} ${measFlagHtml(pass)}</div></div></div>`;
        }
        if (field.type === "number" && (field.min != null || field.max != null)) {
          const raw = fieldStoredValue(v);
          const pass = fieldStoredPass(v, field);
          ...
          return `<div class="diag-field-row"><div class="field"><label>${esc(field.label)}</label>
            <div class="ro-val">${esc(raw)}${unit} ${measFlagHtml(pass)}</div>
            <div class="meas-range muted">Good: ${esc(formatMeasRange(field))}</div></div></div>`;
        }
        return `<div class="diag-field-row"><div class="field ${field.type === "textarea" ? "full" : ""}"><label>${esc(field.label)}</label>
          <div class="ro-val">${esc(fieldStoredValue(v))}</div></div></div>`;
      }).join("");
      const note = values[`s${si}note`];
      const stepClass = stepFail > 0 ? "step-fail" : (stepPass > 0 ? "step-pass" : "");
      const stepStatus = stepFail > 0 ? measFlagHtml(false) : (stepPass > 0 ? measFlagHtml(true) : `<span class="meas-flag"></span>`);
      return `<div class="panel diag-step ${stepClass} ${stepDone ? "step-done" : ""}">
        <div class="diag-step-head">
          <span class="step-num">${si + 1}</span>
          <div class="diag-step-title-wrap">
            <div class="step-title">${esc(s.title)}${stepDone ? " ✓" : ""}</div>
          </div>
          <div class="diag-step-status">${stepStatus}</div>
        </div>
        <div class="diag-fields">${rows}
          ${note ? `<div class="diag-field-row"><div class="field full"><label>Notes</label><div class="ro-val muted">${esc(note)}</div></div></div>` : ""}
        </div></div>`;
    }).join("")}`;
...
function fieldRowHtml(field = {}) {
  ...
  const opts = FIELD_TYPES.map(([v, l]) => `<option value="${v}" ${v === type ? "selected" : ""}>${l}</option>`).join("");
  return `<div class="se-field-row type-${type}">
    <input class="sef-label" placeholder="Input label (e.g. Resistance PWR–GND)" value="${esc(field.label)}">
    <select class="sef-type" onchange="flowFieldTypeChange(this)">${opts}</select>
    <input class="sef-unit" placeholder="unit" value="${esc(field.unit || "")}" title="Number fields: unit shown next to input (e.g. Ω, V)">
    <input class="sef-min" type="number" step="any" placeholder="good ≥" value="${field.min ?? ""}" title="Measurement: lowest acceptable reading">
    <input class="sef-max" type="number" step="any" placeholder="good ≤" value="${field.max ?? ""}" title="Measurement: highest acceptable reading">
    <button type="button" class="li-del" onclick="this.closest('.se-field-row').remove()">&times;</button>
  </div>`;
}
window.flowFieldTypeChange = (sel) => {
  const row = sel.closest(".se-field-row");
  row.className = "se-field-row type-" + sel.value;
};
...
async function flowEditor(id) {
  let f = id ? null : { ...DEFAULT_FLOW_TEMPLATE, steps: JSON.parse(JSON.stringify(DEFAULT_FLOW_TEMPLATE.steps)) };
  if (id) f = (await db.from("diagnostic_flows").select("*").eq("id", id).single()).data;
  const steps = (f.steps && f.steps.length) ? f.steps : DEFAULT_FLOW_STEPS;
  el("view").innerHTML = `
    ...
    <form id="flow-form">
      <div class="panel"><div class="form-grid">
        <div class="field"><label>Title *</label><input name="title" required value="${esc(f.title || "")}" placeholder="e.g. ECU board inspection & fault diagnosis"></div>
        <div class="field"><label>Category</label><input name="category" value="${esc(f.category || "")}" placeholder="e.g. ECU repair, immobiliser, gearbox"></div>
      ...
      <div class="panel">
        <h3>Steps</h3>
        <div class="muted" style="font-size:12px;margin-bottom:10px">Each step is a checklist item with instructions. Add measurements (with good ranges for auto ✓/✗), pass/fail checks, text, or photos.</div>
  ...
  $("#flow-form").addEventListener("submit", async (e) => {
    ...
    const stepsOut = [...document.querySelectorAll("#steps-body .step-edit-row")].map(r => {
      const fields = [...r.querySelectorAll(".se-field-row")].map(fr => {
        ...
        const unit = fr.querySelector(".sef-unit").value.trim();
        if (type === "number" && min !== "") o.min = Number(min);
        if (type === "number" && max !== "") o.max = Number(max);
        if (type === "number" && unit !== "") o.unit = unit;
        return o;
      }).filter(x => x.label);
